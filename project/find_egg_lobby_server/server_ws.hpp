#ifndef GAME_MASTER_SERVER_WS_HPP
#define GAME_MASTER_SERVER_WS_HPP
...
namespace GameMasterServer {
  ...
  public:
     ...
     *   auto connection=std::make_shared<GameMasterServer::SocketServer<GameMasterServer::WS>::Connection>(std::move(socket));
    ...
    void write_handshake(const std::shared_ptr<Connection> &connection) {
      for(auto &regex_endpoint : endpoint) {
        ...
        if(regex::regex_match(connection->path, path_match, regex_endpoint.first)) {
          ...
          if(status_code != StatusCode::information_switching_protocols)
            ostream << "HTTP/1.1 " + GameMasterServer::status_code(status_code) + "\r\n\r\n";
...
#endif /* GAME_MASTER_SERVER_WS_HPP */
