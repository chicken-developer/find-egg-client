...
typedef GameMasterServer::SocketServer<GameMasterServer::WS> WsServer;
typedef GameMasterServer::SocketClient<GameMasterServer::WS> WsClient;
...
int main() {
  ...
  echo.on_message = [&server_callback_count](shared_ptr<WsServer::Connection> connection, shared_ptr<WsServer::InMessage> in_message) {
    ...
    connection->send(in_message_str, [](const GameMasterServer::error_code &ec) {
      if(ec) {
  ...
  for(size_t i = 0; i < 400; ++i) {
    ...
    client.on_error = [](shared_ptr<WsClient::Connection> /*connection*/, const GameMasterServer::error_code &ec) {
      cerr << ec.message() << endl;
      ASSERT(false);
    };
